...
public class dao {
	...
	public ArrayList<testDB_dto> insertTestDB(String sname, String stitle, String scontents){
	
	ArrayList<testDB_dto> arr = new ArrayList<>();
	int sno = 1;
	String query = "insert into test VALUES ("+ sno +",'"+ sname +"','" + stitle + "','" + scontents +"')";
	try{
		connection = common.getConnection();
		ps = connection.prepareStatement(query);
		rs = ps.executeQuery();
		...
		while(rs.next()) {
			...
			String name = rs.getString(2);//name
			String title = rs.getString("title");//3
			String contents = rs.getString("contents");//4
			
			testDB_dto dto = new testDB_dto(no, name, title, contents);
			arr.add(dto);
