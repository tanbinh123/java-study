...
<% 
	...
	ArrayList<News_dto> arr = (ArrayList<News_dto>)request.getAttribute("t_dtos");
...
<script type="text/javascript">
	function formSearch(){
		...
		notiForm.action="/NewsList";
		notiForm.submit();
	}
	function goView(no){
		view.t_no.value= no;
		view.method="post";
		view.action="/NewsView";
		view.submit();
	}
...
<%				if(arr.size() > 0){
					for(int i = 0 ; i < arr.size(); i++ ){ 
						...
						<tr>
							<td><a href="/NewsView?t_no=<%=arr.get(i).getNo()%>"><%=arr.get(i).getNo()%></td>
							<td class="txt"><a href="javascript:goView('<%=arr.get(i).getNo()%>')"><%=arr.get(i).getTitle() %></a></td>
