...
<c:choose>
	<c:when test="${session_name eq null }">
<!--
	 				<li><a href="/member/member_join.jsp">Contact</a></li>
					<li><a href="/member/member_login.jsp">Login</a></li>
					-->
	 				<li><a href="/Member?t_gubun=contact">Contact</a></li>
					<li><a href="/Member?t_gubun=loginForm">Login</a></li>
<%//					} else { %>
	</c:when>
	<c:otherwise>

<!-- 				<li><a href="/member/member_logout.jsp">LogOut</a></li>  -->
					<li><a href="/Member?t_gubun=logout">LogOut</a></li>
	</c:otherwise>
</c:choose>
<%	//				}  %>
				...
				<ul>
					<li class="noti_title">
						<a href="javascript:goView('${arr.getNo()}')">	
							${arr.getRownum()}.
							<c:choose>	
								<c:when test="${fn:length(arr.getTitle()) > 14}">
									${fn:substring(arr.getTitle(),0,14)}...
								</c:when>
								<c:otherwise>
									${arr.getTitle()}
								</c:otherwise>				
					...
					<li class="noti_date">${arr.getReg_date()}</li>
				</ul>
				</c:forEach>						
			</div>
