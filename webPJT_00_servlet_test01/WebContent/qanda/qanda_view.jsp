...
<script type="text/javascript">
	...
	function goQueUpdateForm(){
		qna.t_gubun.value="queupdateForm";
		qna.method="post";
		qna.action="/Qna";
		qna.submit();
	}
	...
	function goAnsUpdateForm(){
		qna.t_gubun.value="ansUpdateForm";
		qna.method="post";
		qna.action="/Qna";
		qna.submit();
	}
...
<%                  if(dto.getAnswer() == null){  //질문자의 답변이 안 달려 있고
						if(dto.getQ_reg_id().equals(session_id)){  //질문자의 아이디가 로그인 아이디와 같으면 수정가능 ||관리자이면%>	
...
<%              if(session_level.equals("top")){   //답변이 달려있고
					if(dto.getAnswer() != null){   //관리자이면 답변 수정, 삭제 가능
%>
