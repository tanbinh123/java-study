...
public class Event_dao {
	...
	public int deleteEvent(String no) {
		int result =0;
		String query =" delete from h00_Event \r\n" + 
					  " where no ='"+no+"'";
		try {
			...
			System.out.println("deleteEvent() query ?ò§Î•?: "+query);
		}catch(Exception ee) {
			System.out.println("deleteEvent() ?ò§Î•?");
		}finally {
	...
	public int updateEvent(Event_dto dto) {
		...
		try {
			...
			System.out.println("updateEvent() query ?ò§Î•?: "+query);
		}catch(Exception ee) {
			System.out.println("updateEvent() ?ò§Î•?");
		}finally {
			common.close(connection, ps, rs);
		}		
		return result;
	...
	public void setHitCount(String no) {
		String query=" update h00_Event set hit = hit + 1\r\n" + 
					 " where no='"+no+"' ";
		try {
			connection = common.getConnection();
			ps  = connection.prepareStatement(query);
			...
			System.out.println("setHitCount() query ?ò§Î•?: "+query);
		}catch(Exception ee) {
			System.out.println("setHitCount() ?ò§Î•?");
		}finally {
			common.close(connection, ps, rs);
		}				
	}	
	...
	public Event_dto getEventView(String no){
		...
		try {
			...
			System.out.println("getEventView() query ø¿∑˘: "+query);
		}catch(Exception ee) {
			System.out.println("getEventView() ø¿∑˘");
		}finally {
			common.close(connection, ps, rs);
		}				
		return dto;
	...
	public ArrayList<Event_dto> getEventList(String select,String search){
		...
		try {
			...
			System.out.println("getEventList() query ø¿∑˘: "+query);
		}catch(Exception ee) {
			System.out.println("getEventList() ø¿∑˘");
		}finally {
			common.close(connection, ps, rs);
		}		
	...
	public int saveEvent(Event_dto dto) {
		int result =0;
		String query ="insert into h00_Event \r\n" + 
		...
		try {
			...
			System.out.println("saveEvent() query ?ò§Î•?: "+query);
		}catch(Exception ee) {
			System.out.println("saveEvent() ?ò§Î•?");
		}finally {
			common.close(connection, ps, rs);
		}		
		return result;
	...
	public String getEventNo() {
		String maxNo="";
		String query=" select max(no) from h00_Event ";
		try {
			connection = common.getConnection();
			ps  = connection.prepareStatement(query);
			rs  = ps.executeQuery();
			...
			System.out.println("getEventNo() query ?ò§Î•?: "+query);
		}catch(Exception ee) {
			System.out.println("getEventNo() ?ò§Î•?");
		}finally {
			common.close(connection, ps, rs);
		}		
		return maxNo;
